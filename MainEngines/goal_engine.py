...
def run_what_if_scenarios(positions_data, current_value, goal, base_volatility):
    scenarios = []
    base_prob = None
    variations = [
    ...
    for r in goal_results:
        ...
        if prob < 40:
            ...
            if needed <= 0:
                nudges.append({
                    "type": "good",
                    "text": f"{goal['name']} is achievable with current pace"})
            else:
                nudges.append({
                    "type": "improve",
                    "text": f"{goal['name']} may need +${int(needed)}/mo"})
        else:
            nudges.append({
                "type": "good",
...
def build_goal_insight(goal_result):
    ...
    return f"+${int(monthly)}/mo → higher success chances"