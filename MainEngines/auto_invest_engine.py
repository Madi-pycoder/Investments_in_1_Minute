...
from ProjectDataBase.cache import AUTO_INVEST_METRICS_CACHE, AUTO_INVEST_INFLIGHT, AUTO_INVEST_LOCKS, CACHE_TTL
    ...
    async with lock:
        ...
        if (fresh_profile.next_auto_invest_at
            ...
            return {
                "ok": False,
                "status": "too_early"}
        success, executed, executed_trades = await rq.execute_rebalance(
            portfolio_id, trades, data["prices_dict"])
        if not success:
            return {
                "ok": False,
