...
class RoboAdvisor:
    ...
    def get_issues(self):
        ...
        if not self.positions:
            issues.append("В портфеле пока нет активов")
        if not self.goals:
            issues.append("🎯 Пока нет финансовых целей")
        return issues
    ...
    def generate_actions(self):
        ...
        if not result["ok"]:
            actions.append({
                "type": "warning",
                "text": "Укажите или увеличьте ежемесячный бюджет"})
        else:
            total = sum(x["amount"] for x in plan)
            if total < 50:
                actions.append({
                    "type": "improve",
                    "text": "Увеличьте сумму регулярных инвестиций"})
        ...
        if goal_analysis:
            for g in goal_analysis:
                ...
                if prob < 40:
                    actions.append({
                        "type": "critical",
                        "text": f"Цель '{g['goal']['name']}' требует внимания"})
        return actions
